...
class ReleaseQualityPageTests(unittest.TestCase):
    ...
    def test_coverage_has_raw_denominator_bounds_and_category_selector(self):
        ...
        for n in 'source_list source_reported_registry_status coverage_class record_count'.split(): self.assertIn(f'mart_registry_population_coverage.{n}',r)
    ...
    def test_no_raw_hash_source_or_named_table(self):
        text=' '.join(json.dumps(load(n)).lower() for n in ('release_health','coverage','last_renewal_diagnostic'))
